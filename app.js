...
async function loadCollections(){var d=await api('/api/collections');collections=d.collections||[];renderColSel()}
function renderColSel(){var sel=document.getElementById('f_collection');if(!sel)return;var opts=collections.map(function(c){return '<option value="'+esc(c)+'">'+esc(c)+'</option>'}).join('');opts+='<option value="__new__">＋ 创建新合集</option>';sel.innerHTML=opts;var saved=document.getElementById('f_catsave').value;if(saved){sel.value=saved}if(sel.value==='__new__'){document.getElementById('f_newcol').style.display='block'}else{document.getElementById('f_newcol').style.display='none'}}
document.getElementById('f_collection').addEventListener('change',function(){document.getElementById('f_newcol').style.display=this.value==='__new__'?'block':'none'});
function newPost(){document.getElementById('f_file').value='';document.getElementById('f_catsave').value='';document.getElementById('f_title').value='';document.getElementById('f_summary').value='';document.getElementById('f_content').value='';document.getElementById('f_newcolname').value='';document.getElementById('f_date').value=nowStr();renderColSel()}
async function openPost(file){var d=await api('/api/post?file='+file);var sel=document.getElementById('f_collection');document.getElementById('f_file').value=d.file;document.getElementById('f_catsave').value=d.categories;document.getElementById('f_title').value=d.title;document.getElementById('f_summary').value=d.description||'';document.getElementById('f_content').value=d.content;document.getElementById('f_date').value=nowStr();if(d.categories&&collections.indexOf(d.categories)<0)collections.push(d.categories);renderColSel();sel.value=d.categories;document.getElementById('f_newcol').style.display=sel.value==='__new__'?'block':'none'}
