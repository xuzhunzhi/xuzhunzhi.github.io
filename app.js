...
function renderColSel(){var sel=document.getElementById('f_collection');if(!sel)return;var opts='<option value="">无合集</option>'+collections.map(function(c){return '<option value="'+esc(c)+'">'+esc(c)+'</option>'}).join('')+'<option value="__new__">＋ 创建新合集</option>';sel.innerHTML=opts;var saved=document.getElementById('f_catsave').value;sel.value=saved||'';document.getElementById('f_newcol').style.display=sel.value==='__new__'?'block':'none'}
